...
function IGCM_Patch_20091030_histcom_time_axis {
    ...
    set +A AXISt $(cat ${1}_ncdump-h | grep 't_ave_.*(time_counter)' | sed -e 's/[[:space:]][fd][lo][ou][ab][tl][ e] *t_ave_\([0-9a-zA-Z]*\).*/\1/')
	...
	for var in ${VARName[*]} ; do 
        ...
        # Rename coordinates attribut from t_ave_xxxx to time_counter

        set +A VARName $( cat ${file}_ncdump-h | grep "[0-9a-zA-Z_]*.*:coordinates.*" | grep "t_ave_${AXISt_i}" | sed -e "s/[[:space:]]*\(.*\):coordinates.*/\1/" )
        for var in ${VARName[*]} ; do
            # Old attribute value
            ATTN=$( cat ${file}_ncdump-h | grep "${var}:coordinates.*" | grep "t_ave_${AXISt_i}" | sed -e "s/.*:coordinates = \"\(.*\)\" ;/\1/" )
            # New attribute value
            ATTNV=$( echo ${ATTN} | sed -e "s/t_ave_${AXISt_i}/time_counter/" ) #| sed -e 's/[[:space:]]/\\ /g')
            # change it ! 
	    IGCM_sys_ncatted -O --hst -a coordinates,${var},m,c,"${ATTNV}" ${file}
        done

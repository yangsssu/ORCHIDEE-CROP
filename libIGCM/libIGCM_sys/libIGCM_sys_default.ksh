...
#**************************************************************
# Author: Sebastien Denvil, Martial Mancip, test2
...
#====================================================
#. /home/${LOGIN}/.atlas_env_${HOST}_ksh
#export PATH=${PATH}:/home/${LOGIN}/rebuild/bin/
...
#====================================================
#- IN
typeset -r R_IN=${R_IN:=/home/${LOGIN}/IGCM}
typeset -r R_IN_ECMWF=${R_IN_ECMWF:=/home/${LOGIN}/IGCM}

#====================================================
#- RUN_DIR_PATH : Temporary working directory (=> TMP)
typeset -r RUN_DIR_PATH=${RUN_DIR_PATH:=/home/${LOGIN}/RUN_DIR/tmp$$}
...
#====================================================
#- set PackDefault to false on the default machine
PackDefault=false
...
function IGCM_sys_defineArchives {
  ...
  # ON DEFAULT MACHINE NO SPECIAL CASE WHEN X${config_UserChoices_SpaceName} = XTEST
...
function IGCM_sys_Qsub {
  ...
  echo "${@} > ${SUBMIT_DIR}/${Script_Output} 2>&1 &"
  ${@} > ${SUBMIT_DIR}/${Script_Output} 2>&1 &
  if [ $? -gt 0 ] ; then
    echo "IGCM_sys_Qsub : erreur ${options} $1"
...
function IGCM_sys_QsubPost {
  ...
  echo "${libIGCM_POST}/$1.job > ${POST_DIR}/$1.${PeriodDateEnd}.out 2>&1 &"
  ${libIGCM_POST}/$1.job > ${POST_DIR}/${Script_Post_Output}.out 2>&1 &
  if [ $? -gt 0 ] ; then
    echo "IGCM_sys_QsubPost : erreur " $@
...
function IGCM_sys_RmRunDir {
  ...
  if ( $DEBUG_sys ) ; then
    ...
    IGCM_debug_Print 1 "Dummy call, let the user do that."
  fi
...
#D-* Purpose: flush buffer on disk (dummy function on Default machine)
#D-* Examples:
#D-
...
function IGCM_sys_build_execution_scripts
{
  ...
  if ( ${OK_PARA_MPMD} ) ; then
    ...
    # MPI-OpenMP (MPMD)
      ...
      i=0
      rm -f hosts
      IGCM_debug_Print 1 "sys $( hostname ), Hosts available :"
  ...
  # Only one executable (SPMD mode).
    ...
    for comp in ${config_ListOfComponents[*]} ; do
      ...
      if ( [ "X${ExeNameOut}" != X\"\" ] && [ "X${ExeNameOut}" != "Xinca.dat" ] ) ; then
        ...
        IGCM_debug_Print 1 "sys $( hostname ) : script_${ExeNameOut}.ksh contains"
  ...
  IGCM_debug_Print 1 "sys $( hostname ) : execution command is"

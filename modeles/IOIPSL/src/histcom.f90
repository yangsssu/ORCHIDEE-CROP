MODULE histcom
!-
!$Id: histcom.f90 2065 2013-05-16 11:56:50Z jgipsl $
!-
...
!-
  ...
  USE mathelp,  ONLY : mathop,moycum,moycum_index,buildop
...
!-
 ...
 &          histwrite,histclo,histsync,ioconf_modname, histglobal_attr
!---------------------------------------------------------------------
...
!-
 ...
 &    'Illegal value of k_typ argument','in internal interface','?')
  ENDIF
!-
  IF (l_dbg) WRITE(ipslout,*) c_nam//" 0.0"
!-
...
!-
  IF (l_dbg) WRITE(ipslout,*) c_nam//" 1.0"
!-
...
!-
  IF (l_dbg) WRITE(ipslout,*) c_nam//" 2.0"
!-
  W_F(idf)%n_var = 0
  W_F(idf)%n_tax = 0
  W_F(idf)%n_hax = 0
...
!-
  IF (l_dbg) WRITE(ipslout,*) c_nam//" 3.0"
!-
...
!-
  IF (l_dbg) WRITE(ipslout,*) c_nam//" 4.0"
!-
...
!-
  IF (l_dbg) WRITE(ipslout,*) c_nam//" 5.0"
!-
  IF (PRESENT(domain_id)) THEN
    W_F(idf)%dom_id_svg = domain_id
  ENDIF
  W_F(idf)%name = TRIM(nc_name)
  W_F(idf)%ncfid = nfid
  IF ( (k_typ == 1).OR.(k_typ == 2) ) THEN
...
!===
SUBROUTINE histglobal_attr &
 & (idf,string_attr,glob_attr)
!---------------------------------------------------------------------
!- Definition of GLOBAL attribute for history files.
!-
!- INPUT
!-
!- idf     : The id of the file to which the grid should be added
!- string_attr : name of the global attribute to be added
!- glob_attr : global_attribute to be added
!---------------------------------------------------------------------
  IMPLICIT NONE
!-
  INTEGER,INTENT(IN) :: idf
  CHARACTER(len=*),INTENT(IN) :: string_attr
  CHARACTER(len=*),INTENT(IN) :: glob_attr
...
!---------------------------------------------------------------------
  ...
  nfid = W_F(idf)%ncfid
  IF (l_dbg) WRITE(ipslout,*) "New global attribute : ",glob_attr
  iret = NF90_PUT_ATT(nfid,NF90_GLOBAL,string_attr,glob_attr)
!-----------------------
END SUBROUTINE histglobal_attr
!===
...
!-
  IF (l_dbg) WRITE(ipslout,*) "histvert : 1.0 Verifications", &
...
!-
  IF (l_dbg) &
 &  WRITE(ipslout,*) "histvert : 2.0 Add the information to the file"
!-
  nfid = W_F(idf)%ncfid
!-
...
!-
  IF (l_dbg) &
  &  WRITE(ipslout,*) "histvert : 3.0 add the information to the common"
!-
  W_F(idf)%n_zax = iv
...
!-
  IF (l_dbg) WRITE(ipslout,*) "histdef : 1.0"
!-
  IF (iv > 1) THEN
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,*) "histdef : 2.0",idf,iv,W_F(idf)%W_V(iv)%nbopp, &
...
!-
 ...
 &    .AND.(TRIM(W_F(idf)%W_V(iv)%topp) /= "never") )THEN
    ...
    W_F(idf)%W_V(iv)%t_bf(:) = missing_val
    IF (l_dbg) THEN
      WRITE(ipslout,*) "histdef : 3.0 allocating time_buffer for", &
...
!-
  IF (l_dbg) WRITE(ipslout,*) "histdef : 4.0"
!-
...
!-
  IF (l_dbg) WRITE(ipslout,*) "histdef : 5.0"
!-
...
!-
  IF (l_dbg) WRITE(ipslout,*) "histdef : 6.0"
!-
...
&      TRIM(str10),ABS(INT(W_F(idf)%W_V(iv)%freq_wrt))
 ...
 &      (TRIM(W_F(idf)%W_V(iv)%topp) /= 'inst')
  ...
  ELSE
    IF (l_dbg) THEN
      WRITE(ipslout,*) "histdef : 7.0 ",TRIM(W_F(idf)%W_V(iv)%topp),'----'
    ENDIF
...
!-
  IF (l_dbg) WRITE(ipslout,*) "histend : 1.0"
!-
...
!-
  IF (l_dbg) WRITE(ipslout,*) "histend : 2.0"
!-
  DO iv=1,W_F(idf)%n_var
!---
...
!-
        IF (l_dbg) THEN
          WRITE(ipslout,*) "histend : 2.0.n, freq_opp, freq_wrt", &
...
!-
  IF (l_dbg) WRITE(ipslout,*) "histend : 3.0"
!-
  iret = NF90_ENDDEF (nfid)
!-
! 4.0 Give some informations to the user
!-
  IF (l_dbg) WRITE(ipslout,*) "histend : 4.0"
!-
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,*) "histwrite 0.0 : VAR : ",W_F(idf)%W_V(iv)%v_name
    WRITE(ipslout,*) "histwrite 0.0 : nbindex :",nbindex
    WRITE(ipslout,*) "histwrite 0.0 : nindex  :",nindex(1:MIN(3,nbindex)),'...'
  ENDIF
!-
...
!-
  IF (.NOT.ALLOCATED(tbf_2)) THEN
    IF (l_dbg) THEN
      WRITE(ipslout,*) "histwrite_real 1.1 allocate tbf_2 ",SIZE(tbf_1)
    ENDIF
    ALLOCATE(tbf_2(W_F(idf)%W_V(iv)%datasz_max))
  ELSE IF (W_F(idf)%W_V(iv)%datasz_max > SIZE(tbf_2)) THEN
    IF (l_dbg) THEN
      WRITE(ipslout,*) "histwrite_real 1.2 re-allocate tbf_2 : ", &
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,*) "histwrite: 3.0",idf
  ENDIF
!-
...
!-
    IF (l_dbg) THEN
      WRITE(ipslout,*) "histwrite: 4.0 tbf_1",idf,iv, &
...
!-
    IF (l_dbg) THEN
      WRITE(ipslout,*) "histwrite: 5.0 idf : ",idf," iv : ",iv," tsz : ",tsz
    ENDIF
!-
...
!-
!------ 5.1 Check if scatter operation is performed
!-      
       flag = .FALSE. 
       DO io = 1, nbopp_max
          IF ( INDEX(TRIM(W_F(idf)%W_V(iv)%sopp(io)),'scatter') > 0 ) THEN 
             flag = .TRUE. 
          END IF
       END DO
       ...
       IF ( flag ) THEN
!-
!------ 5.2  Enter moycum_index only if a scatter operation is performed
!-         
          IF (l_dbg) &
               & WRITE(ipslout,*) "histwrite: 5.2 moycum_index",nbindex,nx,ny,nz
          CALL moycum_index(tmp_opp, W_F(idf)%W_V(iv)%t_bf, &
 &             tbf_2, W_F(idf)%W_V(iv)%nb_opp, nbindex, nindex)
       ELSE
!-
!------ 5.3  Enter moycum otherwise
!-
          IF (l_dbg) &
               & WRITE(ipslout,*) "histwrite: 5.3 moycum",nbindex,nx,ny
...
!-
  IF (l_dbg) WRITE(ipslout,*) "histwrite: 6.0",idf
!-
  IF (do_write) THEN
!-
    nfid = W_F(idf)%ncfid
...
!-
    IF (l_dbg) WRITE(ipslout,*) "histwrite: 6.1",idf
!-
    IF (     (TRIM(tmp_opp) /= "inst") &
 &      .AND.(TRIM(tmp_opp) /= "once") ) THEN
...
!-
 ...
 &      .AND.(TRIM(tmp_opp) /= "once") ) THEN
!-
      IF (l_dbg) WRITE(ipslout,*) "histwrite: 6.2",idf
!-
      itax  = W_F(idf)%W_V(iv)%t_axid
...
!-
    IF (l_dbg) THEN
      WRITE(ipslout,*) "histwrite: 6.3",idf,nfid,nvid,iv,itime
    ENDIF
!-
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,*) 'histvar_seq, start of the subroutine :',learning(idf)
  ENDIF
!-
...
!-
 ...
 &       ' contact the IOIPSL team. ')
        WRITE(ipslout,*) 'The sequence we have found up to now :'
        WRITE(ipslout,*) varseq(idf,1:sp-1)
        varseq_err(idf) = -1
      ENDIF
!-
...
!-
 ...
 &  TRIM(pvarname))
      ...
      IF (l_dbg) &
           WRITE(ipslout,*) "Error history file ",W_F(idf)%name," names : ", &
 &           TRIM(W_F(idf)%W_V(idv)%v_name),TRIM(pvarname)," id : ",idv
    ELSE
!-
...
!-
  DO ifile=i_s,i_e
    IF (W_F(ifile)%ncfid > 0) THEN
      IF (l_dbg) THEN
        WRITE(ipslout,*) '  histsync - synchronising file number ',ifile
      ENDIF
...
!-
  DO ifile=i_s,i_e
    IF (W_F(ifile)%ncfid > 0) THEN
      IF (l_dbg) THEN
        WRITE(ipslout,*) '  histclo - closing specified file number :',ifile
      ENDIF
...
!-----
      IF (l_dbg) THEN
        WRITE(ipslout,*) '  Entering loop on vars : ',W_F(ifile)%n_var
      ENDIF
      DO iv=1,W_F(ifile)%n_var
!------ Extrema
        IF (W_F(ifile)%W_V(iv)%hist_wrt_rng) THEN
          IF (l_dbg) THEN
            WRITE(ipslout,*) 'min value for file :',ifile,' var n. :',iv, &
 &                     ' is : ',W_F(ifile)%W_V(iv)%hist_minmax(1)
            WRITE(ipslout,*) 'max value for file :',ifile,' var n. :',iv, &
...
!---- 2. Close the file
!-----
      IF (l_dbg) WRITE(ipslout,*) '  close file :',nfid

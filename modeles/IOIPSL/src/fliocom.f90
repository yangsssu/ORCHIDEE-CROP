MODULE fliocom
!-
!$Id: fliocom.f90 1896 2012-10-17 15:28:07Z jgipsl $
!-
...
!!--------------------------------------------------------------------
 ...
 &  flio_max_files=100, flio_max_dims=15, flio_max_var_dims=5
  INTEGER,PARAMETER,PUBLIC :: &
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,*) "->fliocrfd - file name : ",TRIM(f_n)
  ENDIF
!-
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,*) '  fliocrfd, external model file-id : ',f_e
  ENDIF
!-
! Create dimensions
  n_u = 0
...
!---
    IF (l_dbg) THEN
      WRITE(ipslout,*) '  fliopstc : Define the Longitude axis'
    ENDIF
!---
...
!---
    IF (l_dbg) THEN
      WRITE(ipslout,*) '  fliopstc : Define the Latitude axis'
    ENDIF
!---
...
!---
    IF (l_dbg) THEN
      WRITE(ipslout,*) '  fliopstc : Define the Vertical axis'
    ENDIF
!---
    k_1=nw_ai(k_lev,f_i);
!---
...
!---
    IF (l_dbg) THEN
      WRITE(ipslout,*) '  fliopstc : Define the Time axis'
    ENDIF
!---
    k_1=nw_ai(k_tim,f_i);
!---
...
!-
  IF (PRESENT(x_axis).OR.PRESENT(x_axis_2d)) THEN
    IF (l_dbg) THEN
      WRITE(ipslout,*) '  fliopstc : Create the Longitude axis'
    ENDIF
    IF (PRESENT(x_axis)) THEN
...
!-
  IF (PRESENT(y_axis).OR.PRESENT(y_axis_2d)) THEN
    IF (l_dbg) THEN
      WRITE(ipslout,*) '  fliopstc : Create the Latitude axis'
    ENDIF
    IF (PRESENT(y_axis)) THEN
...
!-
  IF (PRESENT(z_axis)) THEN
    IF (l_dbg) THEN
      WRITE(ipslout,*) '  fliopstc : Create the Vertical axis'
    ENDIF
...
!-
  IF (PRESENT(t_axis)) THEN
    IF (l_dbg) THEN
      WRITE(ipslout,*) '  fliopstc : Create the Time axis'
    ENDIF
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,*) "->fliodefv ",TRIM(v_n)," ",n_d,"D"
  ENDIF
!-
! Retrieve the external file index
...
!?  ELSE IF (PRESENT(i_15)) THEN; cvr_d = "I4 5D";
    ...
    ENDIF
    WRITE(ipslout,*) "->flioputv ",TRIM(v_n)," ",TRIM(cvr_d)
  ENDIF
!-
! Retrieve the external file index
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,*) "->flioputa ",TRIM(v_n)," ",TRIM(a_n)
  ENDIF
!-
! Retrieve the external file index
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,*) '->flioopfd, file name : ',TRIM(f_n)
  ENDIF
!-
! Search for a free local identifier
!-
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,*) '  flioopfd, model file-id : ',f_e
  ENDIF
!-
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,'("   flioopfd - dimensions :",/,(5(1X,I10),:))') &
 &    nw_dl(:,f_i)
    WRITE(ipslout,*) "<-flioopfd"
  ENDIF
!----------------------
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,'("   fliogstc - dimensions :",/,(5(1X,I10),:))') &
 &    m_x,m_y,m_z,m_t
  ENDIF
!-
...
!---
    IF (l_dbg) THEN
      WRITE(ipslout,*) '  fliogstc - get time details'
    ENDIF
!---
...
!---
    IF (l_dbg) THEN
      WRITE(ipslout,*) '  fliogstc - first time : ',t_axis(1:1)
    ENDIF
  ENDIF
!-
...
!-- Find the calendar
 ...
 &      j_yy,c_1,j_mo,c_1,j_dd,c_1,j_hh,c_1,j_mn,c_1,j_ss
    ...
    CALL lock_calendar (new_status=l_tmp)
    IF (l_dbg) THEN
      WRITE(ipslout,*) '  fliogstc - time_type : '
      WRITE(ipslout,*) it_t
      WRITE(ipslout,*) '  fliogstc - year month day second t_init : '
      WRITE(ipslout,*) j_yy,j_mo,j_dd,r_ss,t_init
    ENDIF
  ENDIF
!-
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,*) "->flioinqv ",TRIM(v_n)
  ENDIF
!-
! Retrieve the external file index
...
!?  ELSE IF (PRESENT(i_15)) THEN; cvr_d = "I4 5D";
    ...
    ELSE IF (PRESENT(r_85)) THEN; cvr_d = "R8 5D";
    ENDIF
    WRITE(ipslout,*) "->fliogetv ",TRIM(v_n)," ",TRIM(cvr_d)
  ENDIF
!-
! Retrieve the external file index
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,*) "->flioinqa ",TRIM(v_n),"-",TRIM(a_n)
  ENDIF
!-
! Retrieve the external file index
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,*) "->fliogeta ",TRIM(v_n)," ",TRIM(a_n)
  ENDIF
!-
! Retrieve the external file index
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,*) "->fliodela ",TRIM(v_n),"-",TRIM(a_n)
  ENDIF
!-
! Retrieve the external file index
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,*) "->fliocpya - file",f_i_i,"-",TRIM(v_n_i),"-",TRIM(a_n)
    WRITE(ipslout,*) "  copied to file ",f_i_o,"-",TRIM(v_n_o)
  ENDIF
!-
! Retrieve the external file index
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,*) "->flioqstc ",TRIM(c_type)
  ENDIF
!-
! Retrieve the external file index
...
!-
  DO i_f=i_s,i_e
    ...
    IF (f_e > 0) THEN
      IF (l_dbg) THEN
        WRITE(ipslout,*) '  fliosync - synchronising file number ',i_f
      ENDIF
      i_rc = NF90_SYNC(f_e)
...
!-
  DO i_f=i_s,i_e
    ...
    IF (f_e > 0) THEN
      IF (l_dbg) THEN
        WRITE(ipslout,*) '  flioclo - closing file number ',i_f
      ENDIF
      i_rc = NF90_CLOSE(f_e)
      nw_id(i_f) = -1
    ELSE IF (PRESENT(f_i)) THEN
...
!-
  WRITE (ipslout,*) 'External model identifier   : ',f_e
  WRITE (ipslout,*) 'Number of dimensions        : ',n_dims
  WRITE (ipslout,*) 'Number of variables         : ',n_vars
  WRITE (ipslout,*) 'ID unlimited                : ',i_unlm
!-
  WRITE (ipslout,*) "---"
  WRITE (ipslout,*) 'Presumed axis dimensions identifiers :'
  IF (n_ai(k_lon) > 0) THEN
    WRITE (ipslout,*) 'x axis : ',n_ai(k_lon)
  ELSE
    WRITE (ipslout,*) 'x axis : NONE'
  ENDIF
  IF (n_ai(k_lat) > 0) THEN
    WRITE (ipslout,*) 'y axis : ',n_ai(k_lat)
  ELSE
    WRITE (ipslout,*) 'y axis : NONE'
  ENDIF
  IF (n_ai(k_lev) > 0) THEN
    WRITE (ipslout,*) 'z axis : ',n_ai(k_lev)
  ELSE
    WRITE (ipslout,*) 'z axis : NONE'
  ENDIF
  IF (n_ai(k_tim) > 0) THEN
    WRITE (ipslout,*) 't axis : ',n_ai(k_tim)
  ELSE
    WRITE (ipslout,*) 't axis : NONE'
  ENDIF
!-
  WRITE (ipslout,*) "---"
  WRITE (ipslout,*) 'Number of global attributes : ',n_atts
  DO k_n=1,n_atts
...
!-
  DO i_n=1,nb_fd_mx
    IF (n_idim(i_n) > 0) THEN
      WRITE (ipslout,*) "---"
      WRITE (ipslout,*) 'Dimension id   : ',n_idim(i_n)
      WRITE (ipslout,*) 'Dimension name : ',TRIM(c_ndim(i_n))
      WRITE (ipslout,*) 'Dimension size : ',n_ldim(i_n)
    ENDIF
  ENDDO
!-
  DO i_n=1,n_vars
 ...
 &           name=c_name,ndims=n_dims,dimids=idimid,nAtts=n_atts)
    WRITE (ipslout,*) "---"
    WRITE (ipslout,*) "Variable name        : ",TRIM(c_name)
    WRITE (ipslout,*) "Variable identifier  : ",i_n
    WRITE (ipslout,*) "Number of dimensions : ",n_dims
    IF (n_dims > 0) THEN
      WRITE (ipslout,*) "Dimensions ID's      : ",idimid(1:n_dims)
    ENDIF
    WRITE (ipslout,*) "Number of attributes : ",n_atts
    DO k_n=1,n_atts
...
!---
    IF (l_dbg) THEN
      WRITE(ipslout,*) "  flio_inf ",kv,ml," ",TRIM(f_d_n)
    ENDIF
!---
...
!---
    IF (i_v < 0) THEN
      ...
      CASE('z')
        ...
        c_r(7)='sigma_level'; c_r(8)='layer';c_r(9)='veget';
      CASE('t')
        n_r = 3

...
!-
 ...
 & qair_obs, pb, lwdown, &
 & eair_obs, zlev_vec, zlevuv_vec, relax !! add  by ke
...
!-
  ...
  REAL, ALLOCATABLE, DIMENSION(:,:) :: veget_init     !! add by ke
  REAL, ALLOCATABLE, DIMENSION(:,:) :: alb_veg_init   !! add by ke
!-
  INTEGER, ALLOCATABLE, DIMENSION(:) :: kindex
  INTEGER, ALLOCATABLE, DIMENSION(:,:) :: nrec_trans  !! add by ke
  INTEGER, ALLOCATABLE, DIMENSION(:,:) :: havlen      !! add by ke
  REAL, ALLOCATABLE, DIMENSION(:,:) :: lon, lat
  REAL, ALLOCATABLE, DIMENSION(:)   :: tmplev
  REAL, ALLOCATABLE, DIMENSION(:)   :: tot_residue    !!  add by Ke
  REAL, ALLOCATABLE, DIMENSION(:)   :: raero          !!  add by ke
  REAL, ALLOCATABLE, DIMENSION(:)   :: t2m_min        !!  add by ke
!-
  REAL :: old_tair
  REAL :: atmco2
  INTEGER :: nbindex
...
!-
  ...
  ALLOCATE (tot_residue(llm))  !! add by ke
  ALLOCATE (raero(llm))        !! add by ke
  ALLOCATE (t2m_min(llm))      !! add by ke
 ...
 &  eair_obs(iim,jjm), zlev_vec(iim,jjm), zlevuv_vec(iim,jjm), relax(iim,jjm)) !! add by ke
!- 
  ALLOCATE &
...
!-
 ...
 &  z0(iim,jjm), qsurf(iim,jjm), nrec_trans(iim,jjm), havlen(iim,jjm), veget_init(iim,jjm),alb_veg_init(iim,jjm))   !! add by ke
!!$, epot_sol(iim,jjm)
!-
...
!-- 
  ...
  kindex(:) = 0
  nrec_trans(:,:) = 0 !! add by ke
  veget_init(:,:) = 0 !! add by ke
  lon(:,:) = 0.0
  lat(:,:) = 0.0
  tmplev(:) = 0.0
...
!-
 ...
 &   kindex, nbindex, force_id)  !! add by ke
!-
...
!-
!=> obsolète ??!! (tjrs calcul?après forcing_read) 
  IF (is_root_prc) THEN
...
!---- and     : for_u, for_v, for_lwdown, for_swnet, for_swdown  !! add by ke
...
!       VOC : radiation
 !!&       for_coszang)
 &       for_coszang,nrec_trans,havlen, raero,t2m_min,date_s2,veget_init,alb_veg_init,tot_residue) !! add by ke
!-------
      IF (printlev_loc>=4) THEN

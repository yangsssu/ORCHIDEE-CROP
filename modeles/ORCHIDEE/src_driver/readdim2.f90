...
MODULE readdim2
!---------------------------------------------------------------------
!< $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/perso/xuhui.wang/MICTcrop/ORCHIDEE/src_driver/readdim2.f90 $ 
  ...
  SUBROUTINE forcing_info(filename, iim, jjm, llm, tm, date0, dt_force,&
    ...
    IF ( interpol ) THEN
       ...
       IF (is_root_prc) THEN
          ...
          ! 
          CALL forcing_vertical(force_id) 
       ELSE
          ALLOCATE(index_l(1))
       ENDIF
       ...
       CALL bcast(jjm_zoom)
       CALL bcast(NbPoint)
       CALL set_grid_glo(iim_zoom,jjm_zoom,NbPoint)
       ...
       WRITE(numout,*) 'In readdim2, : standard PRINTLEV= ', printlev
       WRITE(numout,*) 'In readdim2, : local PRINTLEV_readdim2= ', printlev_loc
        ...
        IF ( contfrac_exists ) THEN
           ...
           DO i=1,iim
              DO j=1,jjm
                 ...
                 IF ( j==jjm .AND. i>ii_end) fcontfrac(i,j)=0.     ! => on mets les données qu'on veut pas du noeud ?missing_value
                 IF ( fcontfrac(i,j) <= EPSILON(1.) ) THEN
                    tair(i,j) = 999999.
                 ENDIF
              ENDDO
  ...
  IF ( foundvar .AND. .NOT. zlevels ) THEN
     ...
     IF ( zheight ) THEN
        ...
        ELSE
           ...
           ENDIF
        ENDIF
        WRITE(numout,*) "forcing_vertical : case ZLEV : Read from forcing file :", zlev_fixed, zlevuv_fixed
        !
     ELSE IF ( zsigma .OR. zhybrid ) THEN
        !
        ...
        ELSE
           ...
           ENDIF
        ENDIF
        WRITE(numout,*) "forcing_vertical : case Pressure coordinates : "
        WRITE(numout,*) "Read from forcing file :", zhybrid_b, zhybrid_a, zhybriduv_b, zhybriduv_a
     ELSE
        !
        ! Why are we here ???
        !
...
!-
  SUBROUTINE flinget_buffer(force_id, varname, iim_full, jjm_full, llm_full, ttm, itb, ite, data_full)
    ...
    !! This subroutine is a wrap of flinget/IOIPSL. The arguments are the same. 
    ...
    IF (first) THEN
       ...
       !Config If   = OFF_LINE
       !Config Help = The full simulation time length will be read if NBUFF equal 0. NBUFF > 1 can be used for smaller regions or site simulations only. 
       !Config Def  = 1
       !Config Units= -
       ...
       END IF
       WRITE(numout,*)'flinget_buffer: NBUFF=',nbuff,' number of time step will be buffered'
       WRITE(numout,*)'flinget_buffer: Choose a lower value for NBUFF if problem with memory'

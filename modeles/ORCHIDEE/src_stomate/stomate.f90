...
!!
!! SVN :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/perso/xuhui.wang/MICTcrop/ORCHIDEE/src_stomate/stomate.f90 $
...
!$OMP THREADPRIVATE(date)
  ...
  INTEGER(i_std), SAVE                           :: date_s2              !! add by ke
!$OMP THREADPRIVATE(date_s2)
...
!$OMP THREADPRIVATE(gslen)
  INTEGER(i_std),    ALLOCATABLE, SAVE, DIMENSION(:, :)      :: havlen   !! add by ke 
!$OMP THREADPRIVATE(havlen)
...
SUBROUTINE stomate_initialize &
        (kjit,           kjpij,             kjpindex,                        &
         ...
         soilc_total, heat_Zimov,           temp_growth,havlen,date_s2) !! add by ke
...
!!!! xuhui: change index to index1 in stomate_initialize because index overwrites the INDEX function of FORTRAN
    ...
    INTEGER(i_std),DIMENSION(kjpindex,nvm), INTENT(out)     :: havlen    !! add by ke
    INTEGER(i_std),INTENT(out)                      :: date_s2           !! add by ke
...
!! end crop variables
        ...
        &         import_yield, t2m_14, litter_not_avail, assim_param,havlen)   !! add by ke
...
!!!!! crop managements, xuhui
    ...
    IF (ok_rotate) THEN
        ...
        ENDIF
	WRITE(numout,*) 'cyc_num_real(:,:) in the var_name = cyc_num part ', cyc_num_real(:,:)
        DEALLOCATE(cyc_num_real)

	WRITE(numout,*) 'cyc_num(:,:) in the var_name = cyc_num part ', cyc_num(:,:)	
        ...
        WRITE(numout,*) " before call stomate_read_cycle, check status of found_restart ",found_restart
        IF (.NOT. found_restart) THEN
	    WRITE(numout,*) 'cyc_num(:,:) before the call of stomate_read_cycle ', cyc_num(:,:)
            CALL stomate_read_cycle(kjpindex,lalo,neighbours,resolution,contfrac)
	    WRITE(numout,*) 'cyc_num(:,:) after the call of stomate_read_cycle ', cyc_num(:,:)
    ...
    WRITE(numout,*) 'dyn_plntdt',dyn_plntdt

    IF ( (.NOT. found_restart) .OR. dyn_plntdt )  THEN  
    !! reading plantdate at each initial dynamic plantdate activated but not rotated
        WRITE(numout,*) 'SP_iplt0 - dyn_plndtd or when not found found_restart',SP_iplt0
        WRITE(numout,*) 'SP_iplt1',SP_iplt1
	WRITE(numout,*) 'cyc_num(:,:) just before the CALL stomate_read_plantdate part ', cyc_num(:,:)

        CALL stomate_read_plantdate(kjpindex,lalo,neighbours,resolution,contfrac)
	WRITE(numout,*) 'cyc_num(:,:) after the CALL stomate_read_plantdate part ', cyc_num(:,:)
    ENDIF ! not found_restart
...
SUBROUTINE stomate_main &
       ...
       &  EndOfYear, f_rot_sech, rot_cmd,nrec_trans,havlen,date_s2,nmatlab,veget_init) !! add by ke
  ...
  !! 0. Variable and parameter declaration
    ...
    INTEGER(i_std),INTENT(in)                       :: kjit              !! Time step number (unitless)
  !!  INTEGER(i_std),INTENT(in)                     :: date              !! add by ke 
    INTEGER(i_std),INTENT(in)                       :: kjpindex          !! Domain size - terrestrial pixels only (unitless)
    INTEGER(i_std),INTENT(in)                       :: kjpij             !! Total size of the un-compressed grid (unitless)
    INTEGER(i_std),INTENT(in)                       :: rest_id_stom      !! STOMATE's _Restart_ file identifier (unitless)
    INTEGER(i_std),INTENT(in)                       :: hist_id_stom      !! STOMATE's _history_ file identifier (unitless)
...
!!!! crops xuhui
    ...
    INTEGER(i_std),DIMENSION(kjpindex,nvm), INTENT(inout)      :: nrec_trans  !! add by ke 
    INTEGER(i_std),DIMENSION(kjpindex,nvm), INTENT(out)        :: havlen      !! add by ke, days since harvesting date
    INTEGER(i_std),INTENT(inout)                               :: date_s2     !! add by ke
    INTEGER(i_std),INTENT(inout)                               :: nmatlab     !! add by ke
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(inout)          :: veget_init  !! add by ke
...
!!!!! end crops, xuhui
  ...
  !                                   ndrpobs(ip, j),                         &  ! IN
                                     ...
                                     codesla,nrec_trans(ip, j), havlen(ip, j),date,nmatlab)  !! add by ke
   
                   lai(ip, j) = slai(ip, j)
              !!!!!!!! add by ke       
                   date_s2 = date
                   
                   if ((date .GT. 366) .AND. (nmat(ip,12) .GT. 0) .AND. (nmatlab .EQ. 0)) then
                      nmatlab = nmat(ip,12)
                      veget_init(ip,:) = 0
                      veget_init(ip,12) = veget(ip,12)
                   else if ((date .GT. 366) .AND. (nmat(ip,12) .GT. 0) .AND. (nmatlab .GT. 0)) then
                      nmatlab = nmatlab
                  !    veget_init(ip,:) = 0
                      veget_init(ip,12) = veget_init(ip,12)
                   else if ((date .GT. 366) .AND. (nmat(ip,12) .EQ. 0) .AND. (nmatlab .GT. 0)) then
                      nmatlab = nmatlab
                  !    veget_init(ip,:) = 0
                      veget_init(ip,12) = veget_init(ip,12)
              ...
              !!!!!!!! add by ke     
                   WRITE(numout,*) 'ke-date_s2', date_s2       !!! add by ke
                   WRITE(numout,*) 'ke-nmat', nmat             !!! add by ke
                   WRITE(numout,*) 'ke-nmatlab', nmatlab       !!! add by ke
                   WRITE(numout,*) 'ke-veget_init', veget_init !!! add by ke
...
!                            f_rot_stom(i,j) = .TRUE.
                        ENDIF
                        WRITE(numout,*) 'f_rot_stom(i,j) before start rotation commands', f_rot_stom(i,j)
                        WRITE(numout,*) 'f_rot_stom(:,:)', f_rot_stom(:,:)

                        IF (f_rot_stom(i,j)) THEN ! start rotation commands
                            !temp_prc = 0.0
                           ! move this part inside the k loop
                            DO k = 1,rot_cmd_max

                               temp_prc = 0.0

                                mycmd = rot_cmd_store(i,k,cyc_num(i,j))
				WRITE(numout,*) 'mycmd in stomate_rotation part', mycmd
				WRITE(numout,*) 'cyc_num(i,j) in stomate_rotation part', cyc_num(i,j)
                                IF (mycmd  .GT. 0) THEN
                                !!!! valid command
                                    CALL stomate_get_cmd(mycmd,stveg,edveg,rotprc)
				    WRITE(numout,*) 'rotprc in stomate_rotation part - after stomate_get_cmd', rotprc
				    WRITE(numout,*) 'stveg in stomate_rotation part - after stomate_get_cmd', stveg
				    WRITE(numout,*) 'j in stomate_rotation part - after stomate_get_cmd', j
				    WRITE(numout,*) 'i in stomate_rotation part - after stomate_get_cmd', i
				    WRITE(numout,*) 'veget_max(i,j) in stomate_rotation part - after stomate_get_cmd', veget_max(i,j)
				    WRITE(numout,*) 'veget_max(:,:) in stomate_rotation part - after stomate_get_cmd', veget_max(:,:)
					...
					WRITE(numout,*) 'matrix_prc_rot(i,stveg,edveg) in stomate_rotation part - after stomate_get_cmd', matrix_prc_rot(i,stveg,edveg)
                            ...
                            IF (temp_prc .GT. 1.0+min_stomate) THEN
                               ...
                               ! WRITE(numout,*) 'temp_prc, rotprc, and rot_cmd_store(:,:,:),rot_cmd_max,cyc_num(:,:) ', temp_prc,rotprc,rot_cmd_store(:,:,:),rot_cmd_max,cyc_num(:,:)
                               ! WRITE(numout,*) 'temp_prc, rotprc, and rot_cmd_store(:,:,:),rot_cmd_max,cyc_num(:,:) ', temp_prc,rotprc,rot_cmd_store(:,:,:)
                                !WRITE(numout,*) 'rot_cmd_max', rot_cmd_max
                                !WRITE(numout,*) 'cyc_num(:,:) ', cyc_num(:,:)

                                WRITE(numout,*) 'temp_prc, rotprc ', temp_prc,rotprc
                                WRITE(numout,*) ' rot_cmd_store(:,:,:) ', rot_cmd_store(:,:,:)
                                WRITE(numout,*) 'rot_cmd_max ', rot_cmd_max
                                WRITE(numout,*) 'cyc_num(:,:) ', cyc_num(:,:)
                                WRITE(numout,*) ' rot_cmd_store(i,k,cyc_num(i,j)) ', rot_cmd_store(i,k,cyc_num(i,j))
                                WRITE(numout,*) ' rot_cmd_store(i,k,:) ', rot_cmd_store(i,k,:)
                                WRITE(numout,*) ' rot_cmd_store(i,:,cyc_num(i,j)) ', rot_cmd_store(i,:,cyc_num(i,j))
                                WRITE(numout,*) 'stveg ', stveg
                                WRITE(numout,*) 'i,j,k ',i,j,k
                                WRITE(numout,*) 'veget_max(i,j)', veget_max(i,j)
                                WRITE(numout,*) 'veget_max', veget_max(:,:)
                                WRITE(numout,*) 'f_rot_stom(i,j)', f_rot_stom(i,j)
                                WRITE(numout,*) 'f_rot_stom(:,:)', f_rot_stom(:,:)

                                WRITE(numout,*) 'nrec(i,j)', nrec(i,j)
                                WRITE(numout,*) 'nrec(:,:)', nrec(:,:)

                                WRITE(numout,*) 'cyc_num(i,j)', cyc_num(i,j)
                                WRITE(numout,*) 'cyc_num(:,:)', cyc_num(:,:)
                                WRITE(numout,*) 'cyc_num_tot(i)', cyc_num_tot(i)

                                WRITE(numout,*) 'cyc_num_tot(:)', cyc_num_tot(:)
...
!          WRITE(numout,*) 'lai after grain: ',lai(1,12:14)
          IF (ok_rotate) THEN
              DO i = 1, kjpindex
		 WRITE(numout,*) 'min_stomate in stomate subroutine, control the call of stomate_rotation, should be smaller ', min_stomate
    		 WRITE(numout,*) 'SUM(matrix_prc_rot(i,:,:)) in stomate subroutine, control the call of stomate_rotation, should be bigger ', SUM(matrix_prc_rot(i,:,:))    
		 WRITE(numout,*) 'matrix_prc_rot(i,:,:) in stomate subroutine, control the call of stomate_rotation', matrix_prc_rot(i,:,:) 
		 WRITE(numout,*) 'f_rot_stom(i,:) in stomate subroutine, control the call of stomate_rotation', f_rot_stom(i,:) 
		 WRITE(numout,*) 'ANY(f_rot_stom(i,:)) in stomate subroutine, control the call of stomate_rotation', ANY(f_rot_stom(i,:)) 
...
!        l_error = l_error .OR. (ier /= 0)
    ...
    ALLOCATE(havlen(kjpindex,nvm),stat=ier)  !! add by ke
        l_error = l_error .OR. (ier /= 0)    !! add by ke
...
!    IF (ALLOCATED(ndrpobs))          DEALLOCATE(ndrpobs)
    ...
    IF (ALLOCATED(havlen)) DEALLOCATE(havlen) !! add by ke
  ...
  SUBROUTINE slowproc_CropVar(nbpt, lalo, neighbours, resolution, contfrac,plantcyc, cropvar)
    ...
    DO it = 1,tml
        DO icyc = 1,1 ! pltcyc ! not dealing with plant cycles for now
			...
			write(*,*) "resolution updated: ", resolution(:,:), "km"
                        write(*,*) "ok_interpol: ", ok_interpol
...
!JG all flio calls must be done by is_root_prc
    IF (is_root_prc) THEN
       ...
       IF (mydebug) THEN
           ...
           WRITE(numout,*) "resolution: ", resolution(1,:) , "m"
       ENDIF
...
!    CALL bcast(lat_rel)
    ...
    IF (ALLOC_ERR/=0) THEN
        ...
        STOP 
    ENDIF
    WRITE(numout,*) "lon_rel(iml,jml) is : ", lon_rel(iml,jml)
    WRITE(numout,*) "lat_rel(iml,jml) is : ", lat_rel(iml,jml)
!    CALL bcast(lon_rel)
    ...
    DO it = 1,tml
        DO ivgt = 1,lml ! ? We can suppose PFTs less than 10 are natural veg without planting date, but not now
            IF (.NOT. natural(ivgt)) THEN
                ...
                nbvmax = 200
		!nbvmax = 2
		WRITE(numout,*) "nbvmax, the maximum amount of fine grids that one coarse grid may have :", nbvmax
                ! the maximum amount of fine grids that one coarse grid may have
...
!!!!-------------------------------------------------------------------------------------------
            ...
            ELSE ! reading rotation map
                ...
                IF ( SIZE(manage,3,i_std) .NE. cyc_rot_max ) THEN
                    ...
                    !WRITE(numout,*) 'rot_1d, cyc_rot_max, manage', rot_1d, cyc_rot_max
                    WRITE(numout,*) 'SIZE(manage,3),manage, cyc_rot_max', SIZE(manage,3,i_std), manage(:,:,:), cyc_rot_max
...
!!!!-------------------------------------------------------------------------------------------
        ...
        IF (.not. iplt_1d) THEN
            ...
            IF ( SIZE(manage,3,i_std) .NE. cyc_rot_max ) THEN
               ! WRITE(numout,*) 'cyc_rot_max, SIZE(manage,3)', cyc_rot_max, SIZE(manage,3,i_std)
                WRITE(numout,*) 'cyc_rot_max, manage, SIZE(manage,3), read PLNTDT part', cyc_rot_max, manage(:,:,:), SIZE(manage,3,i_std)

                STOP 'PLNTDT input and specified maximum rotation cycle not matched'
            ENDIF
	    ...
	    WRITE(numout,*) 'manage(:,:,:) ', manage(:,:,:)
            ...
            DO j = 2,nvm
                ...
                ENDIF
            ENDDO
	    WRITE(numout,*) 'plantdate(:,:,:) ', plantdate(:,:,:) 
        ELSE ! iplt_1d
            IF (nvm_plnt) THEN
        ...
        IF (ok_rotate) THEN
			...
			WRITE(numout,*) 'ip inside the loop ', ip
			WRITE(numout,*) 'j inside the loop ', j			
			WRITE(numout,*) 'plantdate_now(ip,j) inside the loop ', plantdate_now(ip,j)
			WRITE(numout,*) 'plantdate_now(:,:) inside the loop ', plantdate_now(:,:)
			WRITE(numout,*) 'plantdate(:,:,:) inside the loop ', plantdate(:,:,:)
                    ENDIF
                ENDDO
            ENDDO
        ELSE
        ...
        ENDIF
	WRITE(numout,*) 'plantdate_now(:,:) ', plantdate_now(:,:)
	WRITE(numout,*) 'cyc_num(:,:) ', cyc_num(:,:)
	WRITE(numout,*) 'ok_rotate ', ok_rotate
  ...
  !!!! --------------------------------------------------------------
	...
	WRITE(numout,*) 'tgt_rot in stomate_get_cmd', tgt_rot
	WRITE(numout,*) 'src_rot in stomate_get_cmd', src_rot
        WRITE(numout,*) 'prc_rot in stomate_get_cmd', prc_rot
...
! INTEGER(i_std)                                :: rngTol = 2
  INTEGER(i_std)                                :: rngTol = 180
  LOGICAL                                       :: f_convert
...
!  temp_cyc = cyc_num
  ...
  DO jtar = 1, nvm
    ...
    WRITE(numout,*) 'matrix_prc(:,:) in stomate subroutine, control the update the cyc_num', matrix_prc(:,:)
	
    WRITE(numout,*) 'SUM(matrix_prc(:,jtar)) in stomate subroutine, control the update the cyc_num', SUM(matrix_prc(:,jtar))
    WRITE(numout,*) 'min_stomate in stomate subroutine, control the update the cyc_num, if min_stomate smaller than SUM(matrix_prc(:,jtar)), then the cyc_num should be updated', min_stomate
    
    IF (SUM(matrix_prc(:,jtar)) .GT. min_stomate) THEN
        DO jsrc = 2, nvm
	    WRITE(numout,*) 'matrix_prc(jsrc,jtar) in stomate subroutine, control the update the cyc_num', matrix_prc(jsrc,jtar)
	    WRITE(numout,*) 'min_stomate in stomate subroutine, control the update the cyc_num, if min_stomate smaller than matrix_prc, then the cyc_num should be updated', min_stomate
            ...
            IF (matrix_prc(jsrc,jtar) .GT. min_stomate) THEN
			    ...
			    WRITE(numout,*) 'in_cycle(ip,jtar) in stomate subroutine to update the cyc_num, it should be F here', in_cycle(ip,jtar)
			    WRITE(numout,*) 'in_cycle(:,:) in stomate subroutine to update the cyc_num', in_cycle(:,:)


			    WRITE(numout,*) 'cyc_num_old(jsrc) in stomate subroutine to update the cyc_num', cyc_num_old(jsrc)
			    WRITE(numout,*) 'jtar in stomate subroutine to update the cyc_num', jtar
			    WRITE(numout,*) 'cyc_num(ip,jtar) in stomate subroutine to update the cyc_num', cyc_num(ip,jtar)
			    WRITE(numout,*) 'cyc_num(:,:) in stomate subroutine to update the cyc_num', cyc_num(:,:)

                        ELSE ! complete of one rotation cycle, back to 1
                            cyc_num(ip,jtar) = 1
			    WRITE(numout,*) 'cyc_num(ip,jtar) in stomate subroutine after finishing 1 cycle', cyc_num(ip,jtar)
			    WRITE(numout,*) 'cyc_num(:,:) in stomate subroutine after finishing 1 cycle', cyc_num(:,:)

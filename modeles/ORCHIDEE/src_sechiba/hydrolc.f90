...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/perso/xuhui.wang/MICTcrop/ORCHIDEE/src_sechiba/hydrolc.f90 $
...
MODULE hydrolc
  ...
  USE sechiba_io        ! for inpout/output
...
!_ ================================================================================================================================
    ...
    ELSE
       CALL hydrolc_snow(kjit, kjpindex, precip_rain, precip_snow, temp_sol_new, soilcap, &
            frac_nobio, totfrac_nobio, vevapnu, vevapsno, snow, snow_age, snow_nobio, snow_nobio_age, &
            tot_melt, snowdepth) !! add by Ke
    END IF
    ...
    ! 
    CALL hydrolc_flood(kjit,kjpindex, vevapnu, vevapflo, flood_frac, flood_res, floodout) 
...
!    WRITE(numout,*) 'irrig xwang: irrigation(1): ',irrigation(1)
    ...
    CALL hydrolc_soil(kjpindex, vevapnu, precisol, returnflow, reinfiltration, irrigation, irrig_demand_ratio, veget_max, tot_melt, mx_eau_var, &  ! added irrig_demand_ratio, veget_max, for crop irrigation
         ...
         & vegstress, shumdiag, litterhumdiag, irrig_fin) ! added irrig_fin by xuhui add by Ke
    ...
    IF ( ok_hdiff ) THEN
      CALL hydrolc_hdiff(kjpindex, veget, tot_bare_soil, ruu_ch, gqsb, bqsb, dsg, dss, dsp) !! add by Ke
    ENDIF
  ...
  !! 2. Output
    ...
    ! If we use the ALMA standards (then 'humrel' is not written)
    IF (almaoutput) THEN
       CALL hydrolc_alma(kjpindex, index, qsintveg, snow, snow_nobio, soilwet)
    ENDIF

    CALL xios_orchidee_send_field("runoff",run_off_tot*one_day/dt_sechiba)
    ...
    CALL xios_orchidee_send_field("snowf",precip_snow*one_day/dt_sechiba)
    ...
    CALL xios_orchidee_send_field("mrros",run_off_tot/dt_sechiba)
    CALL xios_orchidee_send_field("irrig_rat",irrig_demand_ratio)
    
    histvar(:)=run_off_tot(:)+drainage(:)
    CALL xios_orchidee_send_field("mrro",histvar/dt_sechiba)
...
!MM veget(:,1) BUG ??!!!!!!!!!!!
       ...
       ENDDO
    ENDDO
    CALL xios_orchidee_send_field("mrsos",histvar)

    ! Total soil moisture
    histvar(:)=mean_bqsb(:)+mean_gqsb(:)
    CALL xios_orchidee_send_field("mrso",histvar)
    CALL xios_orchidee_send_field("SubSnow",vevapsno/dt_sechiba)
    CALL xios_orchidee_send_field("SnowDepth",snowdepth)
    CALL xios_orchidee_send_field("Qs",run_off_tot/dt_sechiba)
    CALL xios_orchidee_send_field("Qsb",drainage/dt_sechiba)
    CALL xios_orchidee_send_field("Qsm",tot_melt/dt_sechiba)
...
!_ ================================================================================================================================
  ...
  SUBROUTINE hydrolc_snow (kjit, kjpindex, precip_rain, precip_snow , temp_sol_new, soilcap,&
       & frac_nobio, totfrac_nobio, vevapnu, vevapsno, snow, snow_age, snow_nobio, snow_nobio_age, &
       & tot_melt, snowdepth) 
...
!_ ================================================================================================================================
  ...
  SUBROUTINE hydrolc_flood (kjit, kjpindex, vevapnu, vevapflo, flood_frac, flood_res, floodout) !! addb y Ke
    ! input scalar 
...
!_ ================================================================================================================================
  ...
  SUBROUTINE hydrolc_soil(kjpindex, vevapnu, precisol, returnflow, reinfiltration, irrigation, irrig_demand_ratio, veget_max, tot_melt, mx_eau_var, &  !added irrig_demand_ratio, veget_max, for crop irrigation, xuhui
       ...
       & humrel, vegstress, shumdiag, litterhumdiag,  irrig_fin) !! add by Ke
...
!                         bqsb(ji,jv) = bqsb(ji,jv) + irrig_fin(ji,jv) 
                 ...
                 ELSE
                     IF (irrig_demand_ratio(ji,jv) .GT. zero) THEN
                         ...
                         bqsb(ji,jv) = bqsb(ji,jv) + irrig_fin(ji,jv)
                     ENDIF
!                    bqsb(ji,jv) = bqsb(ji,jv) + irrigation(ji)/(vegtot(ji)-tot_bare_soil(ji)) !! addb y Ke
                 ENDIF
              ENDIF
           ENDIF
...
!_ ================================================================================================================================   
  ...
  SUBROUTINE hydrolc_waterbal (kjpindex, index, veget, totfrac_nobio, qsintveg, snow, snow_nobio,&
       ...
       & vevapsno, vevapflo, floodout, run_off_tot, drainage) !! add by Ke
...
!_ ================================================================================================================================
  ...
  SUBROUTINE hydrolc_hdiff(kjpindex, veget, tot_bare_soil, ruu_ch, gqsb, bqsb, dsg, dss, dsp) !! add by Ke
...
!$OMP THREADPRIVATE(tau_hdiff)
    LOGICAL, SAVE                                        :: firstcall_hydrolc=.TRUE. !! Flag to set tau_hdiff at the first time step 
                                                                             !! ! *** why not define this parameter in 
                                                                             !! constantes_soil.f90 ?
!$OMP THREADPRIVATE(firstcall_hydrolc)
!_ ================================================================================================================================

  !! 1. First time step: we assign a value to the time scale for horizontal water diffusion (in seconds) 
    
    IF ( firstcall_hydrolc ) THEN

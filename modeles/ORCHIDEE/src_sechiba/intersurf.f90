...
!_ ==============================================================================================================================
  ...
  SUBROUTINE intersurf_initialize_2d (kjit, iim, jjm, kjpindex, kindex, xrdt, &
       ...
       tsol_rad, temp_sol_new, qsurf, albedo, emis, z0,nrec_trans,havlen,date_s2,veget_init,alb_veg_init,tot_residue)  !! add by ke
    ...
    CALL sechiba_initialize( &
         ...
         znetco2,      zcarblu,      ztemp_sol_new, zcdrag, nrec_trans,havlen,date_s2,veget_init, alb_veg_init,tot_residue) !! add by ke
...
!_ ==============================================================================================================================
  ...
  SUBROUTINE intersurf_main_2d (kjit, iim, jjm, kjpindex, kindex, xrdt, &
       ...
       coszang, nrec_trans,havlen,raero,t2m_min,date_s2,veget_init,alb_veg_init,tot_residue)   !! add by ke
    ...
    REAL(r_std),DIMENSION (iim,jjm), INTENT(out)            :: fluxlat       !! Latent chaleur flux
    REAL(r_std),DIMENSION (iim,jjm), INTENT(out)            :: emis          !! Emissivity
    INTEGER(i_std),  DIMENSION (iim,jjm), INTENT(out)       :: nrec_trans    !! add by ke  
    REAL(r_std),DIMENSION (iim,jjm), INTENT(out)            :: veget_init    !! add by ke
    INTEGER(i_std),  DIMENSION (iim,jjm), INTENT(out)       :: havlen        !! add by ke, days since harveting date
    REAL(r_std),DIMENSION (kjpindex), INTENT (out)        :: tot_residue     !! add by ke 
    REAL(r_std),DIMENSION (kjpindex, nvm), INTENT (out)   :: alb_veg_init    !! add by ke
...
!_ ==============================================================================================================================
  ...
  SUBROUTINE intersurf_initialize_gathered (kjit, iim_glo, jjm_glo, kjpindex, kindex, xrdt, &
       ...
       field_out_names, fields_out, field_in_names, fields_in, nrec_trans,havlen,date_s2,veget_init,alb_veg_init,tot_residue) !! add by ke
  ...
  SUBROUTINE intersurf_main_gathered (kjit, iim_glo, jjm_glo, kjpindex, kindex, xrdt, &
    ...
    IF (l_first_intersurf) THEN
       ...
       CALL intersurf_initialize_gathered (kjit, iim_glo, jjm_glo, kjpindex, kindex, xrdt, &
            ...
            zlev,  u, v, qair, temp_air, epot_air, &
            cdrag, petAcoef, peqAcoef, petBcoef, peqBcoef, &
            precip_rain, precip_snow, lwdown, swnet, swdown, pb, & !! add by ke
            vevapp, fluxsens, fluxlat, coastalflow, riverflow, &
            tsol_rad, temp_sol_new, qsurf, albedo, emis, z0, lon_scat_g, lat_scat_g, q2m, t2m, &
            field_out_names, fields_out, field_in_names, fields_in,nrec_trans,havlen,date_s2,veget_init,alb_veg_init,tot_residue) !! add by ke  

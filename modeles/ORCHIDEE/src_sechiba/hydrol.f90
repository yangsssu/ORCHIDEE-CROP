...
!$OMP THREADPRIVATE(tot_watveg_end) 
  ...
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:)      :: tot_watres_beg   !! add by ke, Total amount of water on vegetation at start of time 
                                                                         !! step @tex $(kg m^{-2})$ @endtex 
!$OMP THREADPRIVATE(tot_watres_beg)                                      
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:)      :: tot_watres_end   !! add by ke,Total amount of water on vegetation at end of time step
                                                                         !!  @tex $(kg m^{-2})$ @endtex 
!$OMP THREADPRIVATE(tot_watres_end) 
...
!$OMP THREADPRIVATE(evap_bare_lim_ns)
  ...
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)    :: evap_residue_lim_ns !! Limitation factor (beta) for residue evaporation 
                                                                         !! per soiltile (used to deconvoluate vevapnu)  
                                                                         !!  (0-1, unitless) 
!$OMP THREADPRIVATE(evap_bare_lim_ns)
...
!$OMP THREADPRIVATE(frac_bare_ns)
  ...
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION (:,:)    :: frac_residue_ns  !!  add by ke, Evaporating residue fraction per soiltile
                                                                         !!  (0-1, unitless)
!$OMP THREADPRIVATE(frac_residue_ns)
...
!                                 evap_bare_lim,  evap_bare_lim_pft, snow,      snow_age,      snow_nobio,       &
                                 ...
                                 mc_layh_s,      mcl_layh_s, tmc_layh_s)   !! add by ke 
...
!      WRITE(numout,*) 'ke-tot_watveg_beg in hydrol_initialize', tot_watveg_beg,tot_watveg_end
      IF (ALL(tot_watveg_beg(:)==val_exp) .OR.  ALL(tot_watsoil_beg(:)==val_exp) .OR. ALL(snow_beg(:)==val_exp)) THEN
            ...
            CALL hydrol_alma(kjpindex, index, .TRUE., qsintveg, snow, snow_nobio, soilwetdummy) !! add by ke
       END IF
...
!          WRITE(numout,*) 'ke-check_waterbal in hydrol_initialize',tot_watveg_beg,tot_watveg_end, qsintveg,snow, snow_nobio,precip_rain, precip_snow, returnflow, reinfiltration, irrigation, tot_melt,vevapwet, transpir, vevapnu, vevapsno, vevapflo, floodout, runoff,drainage
          CALL hydrol_waterbal(kjpindex, index, .TRUE., veget_max, &
               ...
               & vevapwet, transpir, vevapnu, vevapsno, vevapflo, floodout, runoff,drainage) !! add by ke
       ENDIF
...
!       & humrel, vegstress, drysoil_frac, evapot, evapot_penm, evap_bare_lim, evap_bare_lim_pft, flood_frac, flood_res, &
       ...
       & do_irrig_reservoir, reservoir_dep_max, irrig_dep_threshold, do_awd, awd_dep, lai) !! add by ke
...
!    REAL(r_std),DIMENSION (kjpindex,nvm), INTENT(inout) :: evap_bare_lim_pft    !! Limitation factor (beta) for bare soil evaporation    
    ...
    REAL(r_std), DIMENSION (kjpindex,nvm)              :: frac_res        !! Fraction(of veget_max) of residue in each vegetation type
...
!pss:-
    ...
    CALL hydrol_vegupd(kjit, kjpindex, veget, veget_max, soiltile, qsintveg, resdist, frac_bare, frac_res) !! add by ke
    
    !
    !! 3.3 computes canopy  ==>hydrol_canop
    CALL hydrol_canop(kjpindex, precip_rain, vevapwet, veget_max, veget, qsintmax, qsintveg,precisol,tot_melt) !! add by ke
    !
...
!    irrig_fin(:,:) = zero
    ...
    CALL hydrol_soil(kjit, kjpindex, veget_max, soiltile, njsc, reinf_slope,  &
...
!         tot_melt,evap_bare_lim, evap_bare_lim_pft, shumdiag, shumdiag_perma, &
    ...
    IF (check_waterbal) THEN
       CALL hydrol_waterbal(kjpindex, index, .FALSE., veget_max, totfrac_nobio, &
            ...
            & irrigation, tot_melt, vevapwet, transpir, vevapnu, vevapsno, vevapflo, floodout, runoff, drainage) !! add by ke
    ENDIF

    !! 4 write out file  ==> hydrol_alma/histwrite(*)
    !
    ! If we use the ALMA standards
    CALL hydrol_alma(kjpindex, index, .FALSE., qsintveg, snow, snow_nobio, soilwet) !! add by ke
...
!_ ================================================================================================================================
  ...
  SUBROUTINE hydrol_finalize( kjit,           kjpindex, rest_id,  vegstress,  &
                              ...
                              snowdz,         snowheat,                       &
                              fwet_out,                                       &
                              snowgrain, drysoil_frac, evap_bare_lim)  !! add by ke
...
!_ ================================================================================================================================
    ...
    CALL restput_p(rest_id, 'evap_residue_lim_ns', nbp_glo, nstm, 1, kjit,  evap_residue_lim_ns, 'scatter',  nbp_glo, index_g)  !! add by ke
...
!    CALL restput_p(rest_id, 'evap_bare_lim_pft', nbp_glo, 1, 1, kjit,  evap_bare_lim_pft, 'scatter',  nbp_glo, index_g)
    ...
    CALL restput_p(rest_id, 'tot_watres_beg', nbp_glo,  1, 1, kjit,  tot_watres_beg, 'scatter',  nbp_glo, index_g) !! add by ke
...
!!!!!! add by ke    
    ALLOCATE (frac_residue_ns(kjpindex,nstm),stat=ier) 
    IF (ier /= 0) CALL ipslerr_p(3,'hydrol_init','Problem in allocate of variable frac_residue_ns','','')
!!!!!! add by ke    
...
!!!!!! add by ke      
    ALLOCATE (evap_residue_lim_ns(kjpindex,nstm),stat=ier) 
    IF (ier /= 0) CALL ipslerr_p(3,'hydrol_init','Problem in allocate of variable evap_residue_lim_ns','','')
!!!!!! add by ke  
...
!    WRITE (numout,*) ' ke-tmc ALLOCATE',tmc
...
!pss-
    ...
    !!! add by ke    
    ALLOCATE (tot_watres_beg(kjpindex),stat=ier)
    IF (ier /= 0) CALL ipslerr_p(3,'hydrol_init','Problem in allocate of variable tot_watres_beg','','')
    
    ALLOCATE (tot_watres_end(kjpindex),stat=ier)
    IF (ier /= 0) CALL ipslerr_p(3,'hydrol_init','Problem in allocate of variable tot_watres_end','','')
    !!! add by ke
...
!       WRITE(numout,*) 'ke-water2infilt in init', water2infilt
       !
       var_name= 'ae_ns'
       IF (is_root_prc) THEN
...
!!!! add by ke
       var_name= 'evap_residue_lim_ns'
       IF (is_root_prc) THEN
          CALL ioconf_setatt_p('UNITS', '?')
          CALL ioconf_setatt_p('LONG_NAME','?')
       ENDIF
       CALL restget_p (rest_id, var_name, nbp_glo, nstm, 1, kjit, .TRUE., evap_residue_lim_ns, "gather", nbp_glo, index_g)
       CALL setvar_p (evap_residue_lim_ns, val_exp, 'NO_KEYWORD', 0.0)
!!!! add by ke       
    ...
    !! 5 get restart values if none were found in the restart file
       ...
       !
       !Config Key   = EVAPNU_RESIDUE
       !Config Desc  = residue evap on each soil if not found in restart
       !Config If    = HYDROL_CWRR  
       !Config Def   = 0.0
       !Config Help  = The initial value of residue evap if its value is not found
       !Config         in the restart file. This should only be used if the model is 
       !Config         started without a restart file.
       !Config Units = [mm]
       !
...
! here cvs_over_veg defines the vegetation cover of the previous timestep, consistently with vegstressv and humrelv
    ...
    !!!!!!!!!!!!! add by ke  
    var_name= 'tot_watres_beg'
    IF (is_root_prc) THEN
       CALL ioconf_setatt_p('UNITS', '?')
       CALL ioconf_setatt_p('LONG_NAME','?')
    ENDIF
    CALL restget_p (rest_id, var_name, nbp_glo, 1, 1, kjit, .TRUE., tot_watres_beg, "gather", nbp_glo, index_g)
    !!!!!!!!!!!!! add by ke 
...
!pss-
    ...
    IF (ALLOCATED  (frac_residue_ns)) DEALLOCATE (frac_residue_ns)          !! add by ke
    ...
    IF (ALLOCATED  (evap_residue_lim_ns)) DEALLOCATE (evap_residue_lim_ns)  !! add by ke
    ...
    IF (ALLOCATED  (tot_watres_beg)) DEALLOCATE (tot_watres_beg)            !! add by ke
    IF (ALLOCATED  (tot_watres_end)) DEALLOCATE (tot_watres_end)            !! add by ke
...
!_ ================================================================================================================================
!_ hydrol_tmc_update
  SUBROUTINE hydrol_tmc_update ( kjpindex, veget_max, soiltile, qsintveg, resdist) !! add by ke
...
!    WRITE(numout,*) 'ke-tmc:',tmc(ji,:),water2infilt,dz,mc
!    WRITE(numout,*) 'ke-tmc_old:',tmc_old(ji,:)
             
    !! 4 check
    IF (check_cwrr) THEN
       DO ji=1,kjpindex
          test(ji) = ABS(SUM(tmc(ji,:)*soiltile(ji,:)) - SUM(tmc_old(ji,:)*resdist(ji,:)) + &
               SUM(qsintveg(ji,:)) - SUM(qsintveg_old(ji,:))) !! sum(soiltile)=1 !! add by ke
  ...
  SUBROUTINE hydrol_rotation_update( ip, kjpindex, rot_matrix, old_veget_max, veget_max, soiltile, qsintveg) !! add by ke
...
!    soil_upd=SUM(ABS(soiltile(:,:)-resdist(:,:))) .GT. zero
    ...
    ENDDO
!    WRITE(numout,*) 'ke-tmc in hydrol_rotation_update', tmc,mc,water2infilt
    humtot(ip) = zero
    DO jst=1,nstm
...
!       DO ji=1,kjpindex
        ji = ip
          test = ABS(SUM(tmc(ji,:)*soiltile(ji,:)) - SUM(tmc_old(:)*resdist(ji,:)) + &
               SUM(qsintveg(ji,:)) - SUM(qsintveg_old(:))) ! sum(soiltile)=1  add by ke
...
!_ hydrol_var_init
  ...
  SUBROUTINE hydrol_var_init (kjpindex, veget, veget_max, soiltile, njsc, &
       ...
       drysoil_frac, qsintveg, mc_layh, mcl_layh, tmc_layh, mc_layh_s, mcl_layh_s, tmc_layh_s)  !! add by ke 
...
!   CALL hydrol_tmc_update ( kjpindex, veget_max, soiltile, qsintveg, resdist) !! add by ke
...
!_ hydrol_canop
  ...
  SUBROUTINE hydrol_canop (kjpindex, precip_rain, vevapwet, veget_max, veget, qsintmax, &
       & qsintveg,precisol,tot_melt) !! add by ke
...
!_ hydrol_vegupd
  ...
  SUBROUTINE hydrol_vegupd(kjit, kjpindex, veget, veget_max, soiltile, qsintveg, resdist, frac_bare, frac_res) !! add by ke
    ...
    INTEGER(i_std), INTENT (in)                           :: kjit             !! add by ke, Time step number 
    ! input fields
    ...
    REAL(r_std), DIMENSION (kjpindex,nvm), INTENT(out)    :: frac_res         !! Fraction(of veget_max) of residue in each vegetation type
...
!_ ================================================================================================================================
  ...
  !  WRITE(numout,*) 'ke-frac_res hydrol_vegupd', frac_res
  !  WRITE(numout,*) 'ke-frac_residue_ns hydrol_vegupd', frac_residue_ns
  !  WRITE(numout,*) 'ke-frac_bare hydrol_vegupd', frac_bare
 !   WRITE(numout,*) 'ke-frac_bare_ns hydrol_vegupd', frac_bare_ns
...
!_ hydrol_soil
  ...
  SUBROUTINE hydrol_soil (kjit, kjpindex, veget_max, soiltile, njsc, reinf_slope, &
    ...
    ! input scalar 
    INTEGER(i_std), INTENT(in)                               :: kjpindex 
    INTEGER(i_std), INTENT(in)                               :: kjit             !! add by ke
    ! input fields
...
!_ ================================================================================================================================
    ...
    CALL hydrol_split_soil (kjit, kjpindex, veget_max, soiltile, vevapnu, vevapnu_pft, transpir, humrel, evap_bare_lim, tot_bare_soil) !! add by ke
    ...
    DO jst = 1,nstm
       ...
       tmcold(:) = tmc(:,jst)
!       WRITE (numout,*) 'ke-tmc-hydrolsoil',tmcold,tmc
       !
...
!                  & /(un-EXP(-humcste(jv)*zz(nslm)/mille))
    ...
    !
    CALL hydrol_diag_soil (kjit, kjpindex, veget_max, soiltile, njsc, runoff, drainage, &
...
!    evap_bare_lim_pft(:,:) = zero
 ...
 !!   evap_residue_lim(:) = zero
 !!   evap_residue_lim_ns(:,:) = zero
...
!_ hydrol_split_soil
  ...
  SUBROUTINE hydrol_split_soil (kjit, kjpindex, veget_max, soiltile, vevapnu, vevapnu_pft, transpir, humrel, evap_bare_lim, tot_bare_soil) !! add by ke
    ! 
    ! interface description
...
!!          ENDIF
 ...
 !   WRITE(numout,*)'ke-vevapnu_ns  hydrol_split_soil',vevapnu_ns
  !  WRITE(numout,*)'ke-ae_ns hydrol_split_soil',ae_ns

 !   WRITE(numout,*)'ke-kjit hydrol_split_soil',kjit
...
!_ hydrol_diag_soil
  ...
  SUBROUTINE hydrol_diag_soil (kjit, kjpindex, veget_max, soiltile, njsc, runoff, drainage, &
       & evapot, vevapnu, returnflow, reinfiltration, irrigation, &
       & shumdiag,shumdiag_perma, k_litt, litterhumdiag, humrel, vegstress, drysoil_frac, tot_melt, & !pss:+
       & drunoff_tot) !pss:- !! add by ke
...
!          ENDIF
    ...
    DO jst = 1, nstm
       ...
       END DO
    END DO
!    WRITE(numout,*) 'ke-tot_watveg_beg in hydrol_soil', tot_watveg_beg,tot_watveg_end,humtot
...
!! \n
!_ ================================================================================================================================
  SUBROUTINE hydrol_waterbal_init(kjpindex, qsintveg, snow, snow_nobio) !! add by ke
...
!_ hydrol_waterbal
  ...
  SUBROUTINE hydrol_waterbal (kjpindex, index, first_call, veget_max, totfrac_nobio, &
    ...
    INTEGER(i_std) :: ji
    REAL(r_std) :: watveg, delta_water,watres
    LOGICAL     :: error=.FALSE.  !! If true, exit in the end of subroutine
...
!_ hydrol_alma
  ...
  SUBROUTINE hydrol_alma (kjpindex, index, first_call, qsintveg, snow, snow_nobio, soilwet) !! add by ke
    !
    !! 0. Variable and parameter declaration
...
!_ ================================================================================================================================
  ...
  !!!! add by Ke     
    ...
    ENDDO
!    WRITE(numout,*) 'ke---tot_watveg_end in hydrol_alma', watveg,tot_watveg_end,tot_watsoil_beg

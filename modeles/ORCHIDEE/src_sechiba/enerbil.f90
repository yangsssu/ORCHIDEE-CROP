...
!add cdrag_pft, veget_max,soilflx_pft, soilcap_pft,  xuhui
    ...
    REAL(r_std), DIMENSION(kjpindex), INTENT(in)         :: t2m_min          !! add by ke, min. 2 m air temp. during forcing time step (K)
    ...
    REAL(r_std),DIMENSION (kjpindex), INTENT(in)         :: t2m              !! add by ke,Surface air temperature
 
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)        :: raero            !! add by ke, Aerodynamic resistance (s m^{-1})
    REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (in)    :: rveget           !! add by ke, Stomatal resistance for the whole canopy (s m^{-1})
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)        :: z0               !! add by ke,surface roughness
...
!!    WRITE(numout,*) 'qsol_sat_new_pft(1,:)', qsol_sat_new_pft(1,:)
    CALL enerbil_surftemp (kjpindex, zlev, emis, &
       ...
       & qair_new, epot_air_new, snowdz, veget_max,nrec_trans,havlen,date_s2) !! add by ke
...
!>>>>>>> .r3384done
    ...
    CALL enerbil_evapveg (kjit, kjpindex, vbeta1, vbeta2, vbeta3, vbeta3pot, vbeta4, vbeta4_pft, vbeta5, veget_max,  &
       & rau, u, v, q_cdrag, q_cdrag_pft, qair_new, humrel, vevapsno, vevapnu , vevapnu_pft, vevapflo, vevapwet, &
       & transpir, transpot, evapot, &
       & snowdz,snowrho,evapot_corr,havlen,pb,q2m,t2m,raero,rveget,t2m_min,z0) !!add by ke
...
!=======
!>>>>>>> .r3384done
  !    WRITE(numout,*) 'ke-soilcap(ji)',soilcap(ji)
   !   WRITE(numout,*) 'ke-soilcap_pft(ji,:)', soilcap_pft(ji,:)
  !    WRITE(numout,*) 'ke-sum_old',sum_old
  !    WRITE(numout,*) 'ke-sum_old_pft',sum_old_pft
  !    WRITE(numout,*) 'ke-sum_sns',sum_sns
  !    WRITE(numout,*) 'ke-sum_sns_pft',sum_sns_pft
  !    WRITE(numout,*) 'ke-soilcap_pft', soilcap_pft
  !    WRITE(numout,*) 'ke-dtheta', dtheta
  !    WRITE(numout,*) 'ke-dtheta_pft', dtheta_pft
  !    WRITE(numout,*) 'ke-psold', psold
  !    WRITE(numout,*) 'ke-psold_pft', psold_pft
...
!_ ================================================================================================================================
    ...
    !! 2. Calculation of the net upward flux of longwave radiation
      ...
      !!WRITE(numout,*) 'ke-fluxlat : ', fluxlat(ji),',',vbeta1(ji),',',vbeta5(ji),',',vbeta(ji),',',valpha(ji),',',qsol_sat_new(ji),',',qair(ji)
  ...
  SUBROUTINE enerbil_evapveg (kjit, kjpindex, vbeta1, vbeta2, vbeta3, vbeta3pot, vbeta4, vbeta4_pft, vbeta5, veget_max,&
...
!!$ DS 15022011 humrel was used in a previous version of Orchidee, developped by Nathalie. Need to be discussed if it should be introduce again             
    ...
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)       :: pb                !! add by ke, Lowest level pressure (hPa)
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)       :: q2m               !! 2m specific humidity
    REAL(r_std),DIMENSION (kjpindex), INTENT(in)        :: t2m               !! add by ke,Surface air temperature
    ...
    REAL(r_std)                                         :: xxtemp
    REAL(r_std), PARAMETER                              :: ro=1.2            !! add by ke
    REAL(r_std), PARAMETER                              :: cp=0.001013       !! add by ke
    REAL(r_std), PARAMETER                              :: const_to = 86400  !! add by ke
 !!   REAL(r_std)                                       :: gamma             !! add by ke
    REAL(r_std), PARAMETER                              :: alpha = 1.32      !! add by ke
    REAL(r_std), PARAMETER                              :: pi = 3.141592653589793238!! add by ke, pi souce : http://mathworld.wolfram.com/Pi.html (unitless)
    REAL(r_std), PARAMETER                              :: Tetens_1 = 0.622         !! add by ke, Ratio between molecular weight of water vapor and molecular weight  
                                                                                    !! of dry air (unitless)
    REAL(r_std), PARAMETER                              :: Tetens_2 = 0.378         !! add by ke, 
    REAL(r_std), PARAMETER                              :: p_zr=2.0,karm=0.41,nconv = 2.5, z0s = 0.005   !! add by ke, m
    REAL(r_std), DIMENSION(kjpindex)                    :: rad_temp1, rad_temp2, rad_temp,deltat,tpm,dsat,dos,ePT,gamma,netradS,raslim,raszero,d,height,ras,L,mouillmulch                                                 !! add by ke, 

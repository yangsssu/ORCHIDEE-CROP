...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE Animaux_main(&
    ...
    !JCADD for autogestion 5 grazing AGB and litter
...
!                END WHERE
            ...
            IF (is_grassland_manag(j) .AND. (is_c4(j)) .AND. &
               (.NOT.is_grassland_cut(j)).AND.(.NOT.is_grassland_grazed(j)))THEN
              ...
              WHERE ((ok_ugb(:,j) .EQ. 0))
                ...
                WHERE (((nb_ani(:,j)-nb_ani_old(:,j))/nb_ani(:,j)) .LT. 0.01 &
                       .AND. (grazed_frac(:,j) .LT. 0.7) .AND. &
                       (sr_ugb(:,j) .GT.0.0))
                  ok_ugb(:,j) = 1
                  sr_ugb(:,j) = sr_ugb(:,j) - 0.00001
                ...
                END WHERE
!JCMODIF Feb2015 for too low sr_ugb balance
!                WHERE ((grazed_frac(:,j) .GT. 0.9).AND.(sr_ugb(:,j) .GT. 0.0))
!                  sr_ugb(:,j) = sr_ugb(:,j) + 0.00002
!                END WHERE
      ...
      IF(f_nonlimitant .EQ. 0) THEN
        ...
        ENDIF
        !JCADD for grazing biomass in summer and litter in winter
      ...
      IF(f_nonlimitant .EQ. 0) THEN
        ...
        print *,  'new year', mgraze_C3
        print *, 'import',import_yield(:,mgraze_C3)
        print *, 'sr',sr_ugb(:,mgraze_C3)
        print *, 'nb',nb_ani(:,mgraze_C3)
        print *, 'frac',grazed_frac(:,mgraze_C3)
          ...
          END WHERE
          print *, 'frac_post',grazed_frac(:,mgraze_C3)
        ...
        !! increased then decreased livestock density 
...
!        ELSEWHERE (wshtot(:,mgraze_C4) .LT. min_grazing)
      ...
      ! 0.01 LSU 5 gDM  
!!! JCADD for global simulation with wild animal grazing natural grassland
...
!!!!!! JCADD for global simulation with wild animal grazing natural grassland
      ...
      ! JCADD for MICT LGM grazing biomass and litter
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  SUBROUTINE Animal_Init(&
    ...
    l_error = .FALSE.
    ALLOCATE (milk              (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (milkn             (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (milkc             (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (ranimal           (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (Methane           (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (faecesnsumprev    (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (milkndaily        (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (faecesndaily      (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (urinendaily       (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (milksum           (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (nelgrazingsum     (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (milkcsum          (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (ranimalsum        (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (Methanesum        (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (urinecsum         (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (faecescsum        (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (urinensum         (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (faecesnsum        (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (milknsum          (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (milknsumprev      (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (urinensumprev     (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (stockingstart     (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (stockingend       (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (wshtotstart       (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (grazingsum        (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (grazingcsum       (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (grazingnsum       (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (grazingc          (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (grazingn          (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (grazingnsumprev   (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (grazingndaily     (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (forage_complementc(npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (forage_complementn(npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (forage_complementcsum(npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (forage_complementnsum(npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (methane_ani       (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (methane_aniSum    (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (milkanimalsum     (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (milkanimal     (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (ugb               (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (ok_ugb            (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (extra_feed        (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (Wanimalcow     (npts,nvm,2),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (BCScow         (npts,nvm,2),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (BCScow_prev    (npts,nvm,2),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (AGEcow         (npts,nvm,2),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (Forage_quantity_period (npts,nvm),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (MPcowCsum      (npts,nvm,2),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (MPcowNsum      (npts,nvm,2),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (MPcowN         (npts,nvm,2),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (MPcowC         (npts,nvm,2),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (MPcowsum       (npts,nvm,2),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (MPcow2sum      (npts,nvm,2),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (MPcow2_prec     (npts,nvm,2),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (DMIcowsum      (npts,nvm,2),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (DMIcowNsum     (npts,nvm,2),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (DMIcowCsum     (npts,nvm,2),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (DMIcowanimalsum (npts,nvm,2),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (Wanimalcalf        (npts,nvm),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (DMIcalfsum         (npts,nvm),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (DMIcalfnsum        (npts,nvm),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (DMIcalfanimalsum   (npts,nvm),stat=ier); l_error=l_error .OR. (ier .NE. 0)   
    ALLOCATE (Tcalving           (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (Tsevrage           (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (Age_sortie_calf    (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (Pyoung             (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (Wcalfborn          (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (calfinit           (npts,nvm),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (Wanimalcalfinit    (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (calf               (npts,nvm),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (nanimaltot_prec    (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (Gestation          (npts,nvm),stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ALLOCATE (compte_pature      (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
...
! Initialisations    
    ...
    END WHERE
    !!!!!!!!
    !JCADD 130418 component selection in animal intake
    !!!!!!!!
...
!print *, 'PlantLam'
   ...
   ELSEWHERE
    ...
    END WHERE
!JCADD 20141121 for avoid over grazing stem and leaf simutaneously
...
!print *, 'PlantEar',PlantEarzeargrazing(:,6)
    !!!!!!!!
    !JCADD 120409 new update leaf_frac for each class
    ...
    ENDDO
!print *,'after frac'
    !!! 05212013 JCADD NDF and DNDF DNDFI in grazed grassland put after grazing
...
!JCADD 08Sep2015 to avoid pstem and plam over 1
        WHERE (pstem .GT. 1.0)
          pstem = 1.0

...
MODULE stomate_glcchange_fh
  ...
  PRIVATE
  PUBLIC gross_glcc_firstday_fh, gross_glcchange_fh, age_class_distr
...
!_ ================================================================================================================================
  ...
  !! 0. Variable and parameter declaration 
    ...
    REAL(r_std), DIMENSION(npts,nvm)                   :: soilcarbon          !! 
...
!_ ================================================================================================================================
 ...
 !! 2. Handle the merge of PFTs when one age class moves to the next one.
    ...
    !  in this age class. The biomass, litter, soilcarbon and soil
...
!>\DESCRIPTION  
!_ ================================================================================================================================
  SUBROUTINE gross_glcchange_fh (npts, dt_days, harvest_matrix,   &
    ...
    WRITE(numout,*) 'Entering gross_glcchange_fh'
    glcc_harvest(:,:) = zero
    glccReal_tmp(:,:) = zero
    ...
    ENDDO
  END SUBROUTINE gross_glcchange_fh
...
!_ ================================================================================================================================
  ...
  SUBROUTINE gross_glcc_firstday_fh(npts,veget_max_org,harvest_matrix, &
                          glccSecondShift,glccPrimaryShift,glccNetLCC,&
                          glccReal,glcc_pft,glcc_pftmtc,IncreDeficit, &
                          Deficit_pf2yf_final, Deficit_sf2yf_final,   &
                          pf2yf_compen_sf2yf, sf2yf_compen_pf2yf)
    ...
    REAL(r_std), DIMENSION(npts,2)          :: vegagec_tree_twocl  !! Forest fraction in two big classes: the oldest and other
                                                                  !! age classes.
    ...
    ! check various purposes, e.g., whether input harvest and glcc matrix 
    ...
    !! 2. Handle the forestry harvest process
    ...
    ! we now have to fill the transtion of forest->forest because of harvest
    ...
    ! [Note: below we used the tempelate of type_conversion but in fact we need
    ...
    ! Thought glcc_pftmtc and glcc_pft_tmp will be automatically filled when 
    ! we use the tempelate type_conversion by calling it as below, however they
    ! will be re-set to zero when handling shifting LCC in and net LCC in later 
    ! sections.]

    !! 2.1.1 Secondary forest harvest within modeled secondary forest age classes.

    ! We first handle within the secondary forest age classes, in the sequence 
    ! of old->young

    IndStart_f = 2             ! note the indecies and vegetfrac for tree age class
                               ! is from old to young, thus index=2 means the
                               ! 2nd oldest age class.
    IndEnd_f = nagec_tree-1    ! the 2nd youngest age class.

    DO ipts=1,npts
      !sf2yf
      CALL type_conversion(ipts,sf2yf,harvest_matrix,veget_mtc, &
    ...
    !! 2.1.2 Use primary forest harvest to compensate the deficit in secondary 
    !!       forest harvest within secondary forest in the model.
    ...
    DO ipts=1,npts
      IF (FHmatrix_remainA(ipts,sf2yf) .GT. zero) THEN
        ...
        !sf2yf
        CALL type_conversion(ipts,sf2yf,FHmatrix_remainA,veget_mtc, &
                         indold_tree,indagec_tree,indagec_pasture,num_pasture_mulagec,&
                         IndEnd_f,nagec_herb,                    &
                         vegagec_tree,veget_max,glcc_pft,glcc_pftmtc,glcc_pft_tmp,&
                         glccRemain, &
    ...
    !! 2.2 Handle primary forest harvest
    
    CALL calc_cover(npts,veget_max,veget_mtc,vegagec_tree,vegagec_grass, &
           vegagec_pasture,vegagec_crop)

    ! we check first if there is still deficit in the required secondary
    ! harvest. If yes, that means all existing forest (except the youngest
    ! age class) is depleted, thus required primary harvest will be suppressed.
    ! Otherwise we will treat primary forest harvest starting from modeled
    ...
    DO ipts=1,npts
      ...
      ELSE
        ...
        ! we treat primary harvest wihtin the modeled oldest age class.

        IndStart_f = 1             ! Oldest age class
        IndEnd_f = nagec_tree-1    ! 2nd youngest age class

        !pf2yf
        CALL type_conversion(ipts,pf2yf,FHmatrix_remainB,veget_mtc, &
                         indold_tree,indagec_tree,indagec_pasture,num_pasture_mulagec,&
                         IndEnd_f,nagec_herb,                    &
                         vegagec_tree,veget_max,glcc_pft,glcc_pftmtc,glcc_pft_tmp,&
                         glccRemain, &
    ...
    ! which vegetation types should stay in here in case there is deficit occuring
    ...
    ! be treated starting from 3rd oldest age class and moving torward older
    ! age class.
    glccRemain(:,:) = glccSecondShift(:,:)

    ! Now we will call type_conversion for each of the 12 transitions, starting
    ! from 2nd age class moving to the youngest age class. We use glccRemain
    ...
    IndStart_f = nagec_tree-2  ! note the indecies and vegetfrac for tree age class
                               ! is from old to young, thus nagec_tree-1 means the
                               ! 3rd youngest age class.
    IndEnd_f = nagec_tree-2    ! nagec_tree-2: The 3rd youngest age class
    ...
    DO ipts=1,npts
      !f2c
      CALL type_conversion(ipts,f2c,glccSecondShift,veget_mtc,       &
                       ...
                       .TRUE., iagec_start=IndStart_f)
      !f2p
      CALL type_conversion(ipts,f2p,glccSecondShift,veget_mtc,       &
                       indold_tree,indagec_tree,indagec_pasture,num_pasture_mulagec,     &
                       IndEnd_f,nagec_herb,                    &
                       vegagec_tree,veget_max,glcc_pft,glcc_pftmtc,glcc_pft_tmp, &
                       glccRemain, &
                       .TRUE., iagec_start=IndStart_f)
      !f2g
      CALL type_conversion(ipts,f2g,glccSecondShift,veget_mtc,       &
                       ...
                       .TRUE., iagec_start=IndStart_f)
      !g2c
      CALL type_conversion(ipts,g2c,glccSecondShift,veget_mtc,       &
                       ...
                       .TRUE.)
      !g2p
      CALL type_conversion(ipts,g2p,glccSecondShift,veget_mtc,       &
                       ...
                       .TRUE.)
      !g2f
      CALL type_conversion(ipts,g2f,glccSecondShift,veget_mtc,       &
                       ...
                       .TRUE.)
      !p2c
      CALL type_conversion(ipts,p2c,glccSecondShift,veget_mtc,       &
                       ...
                       .TRUE.)
      !p2g
      CALL type_conversion(ipts,p2g,glccSecondShift,veget_mtc,       &
                       ...
                       .TRUE.)
      !p2f
      CALL type_conversion(ipts,p2f,glccSecondShift,veget_mtc,       &
                       ...
                       .TRUE.)
      !c2p
      CALL type_conversion(ipts,c2p,glccSecondShift,veget_mtc,       &
                       ...
                       .TRUE.)
      !c2g
      CALL type_conversion(ipts,c2g,glccSecondShift,veget_mtc,       &
                       ...
                       .TRUE.)
      !c2f
      CALL type_conversion(ipts,c2f,glccSecondShift,veget_mtc,       &
    ...
    IndStart_f = nagec_tree-3  ! note the indecies and vegetfrac for tree age class
                               ! is from old to young, thus nagec_tree-2 means the
                               ! 3rd oldest age class.
    IndEnd_f = 1
    ...
    DO ipts=1,npts
      !f2c
      CALL type_conversion(ipts,f2c,glccSecondShift_remain,veget_mtc,       &
                       indold_tree,indagec_tree,indagec_crop,num_crop_mulagec,     &
                       IndEnd_f,nagec_herb,                    &
                       vegagec_tree,veget_max,glcc_pft,glcc_pftmtc,glcc_pft_tmp, &
                       glccRemain, &
      ...
      !f2p
      CALL type_conversion(ipts,f2p,glccSecondShift_remain,veget_mtc,       &
                       indold_tree,indagec_tree,indagec_pasture,num_pasture_mulagec,     &
                       IndEnd_f,nagec_herb,                    &
                       vegagec_tree,veget_max,glcc_pft,glcc_pftmtc,glcc_pft_tmp, &
                       glccRemain, &
                       .FALSE., iagec_start=IndStart_f)
      !f2g
      CALL type_conversion(ipts,f2g,glccSecondShift_remain,veget_mtc,       &
                       indold_tree,indagec_tree,indagec_grass,num_grass_mulagec,     &
                       IndEnd_f,nagec_herb,                    &
                       vegagec_tree,veget_max,glcc_pft,glcc_pftmtc,glcc_pft_tmp,&
                       glccRemain, &
    ...
    !+++ Code freezing: Compensation among different transition fractions +++
    !+++ Description: This block of code and associated subroutines are originally
    !+++ developed to make the LCC module compatible with DGVM.
    !
    !! we copy updated veget_max to veget_max_tmp.
    !! The latter will be used to retrieve the values of veget_max after checking 
    !! the consistency of input glcc with existing vegetation fractions.
    ...
    IncreDeficit(:,:) = IncreDeficit(:,:) - glccRemain(:,:)

  END SUBROUTINE gross_glcc_firstday_fh
...
!_ ================================================================================================================================
  SUBROUTINE cross_give_receive(ipts,frac_used,veget_mtc,                     &
    ...
    ![20160130 note here totalveg is the total fraction of all existing MTCs
    ! that are going to recieve newly convervted fractions.]
    totalveg = 0.
...
!_ ================================================================================================================================
  SUBROUTINE calc_cover(npts,veget_max,veget_mtc,vegagec_tree,vegagec_grass, &
    ...
    DO ivma = 1,nvmap
      ...
      ELSE
        veget_mtc(:,ivma) =           SUM(veget_max(:,staind:staind+nagec_pft(ivma)-1),DIM=2)
      ENDIF
    ENDDO
...
END MODULE stomate_glcchange_fh

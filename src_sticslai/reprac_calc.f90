...
subroutine reprac_calc(n, nsen, nlax, nflo, nmat, nrec, nplt, in_cycle, nger, tcult, nlev, &      ! IN
                     ...
                     P_codetemprac, P_tcmin, P_tcmax, P_codedyntalle, P_tcxstop, P_stlevamf, P_stamflax, &
                     N_impact)     ! parameter XZ ADD
...
real,    intent(in)                :: N_impact       ! // parameter XZ ADD
...
!!        XZ ADD
          if (ok_Nlim) then
                reprac = ((repracmax-repracmin) * (exp(-kreprac * (urac - 1.))) + repracmin) * N_impact
          else

...
!$OMP THREADPRIVATE(pref_soil_veg)
  ...
  ! BNF
  LOGICAL, ALLOCATABLE, SAVE, DIMENSION(:) :: is_legume             !! flag for legume vegetation types (true/false)
!$OMP THREADPRIVATE(is_legume)
  ! PHOTOSYNTHESIS
  !
  !-
  ! 1. CO2
...
!$OMP THREADPRIVATE(SP_codgellev)
  INTEGER(i_std), ALLOCATABLE, SAVE, DIMENSION(:) :: SP_codgeljul
!$OMP THREADPRIVATE(SP_codgeljul)

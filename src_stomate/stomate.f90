...
!  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:)    :: resp_hetero_litter_d
  ...
  !! XZ ADD
  REAL(r_std), DIMENSION(:,:), ALLOCATABLE, SAVE :: N_impact
...
!$OMP THREADPRIVATE(N_add)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:, :)       :: Nbnf
!$OMP THREADPRIVATE(Nbnf)
...
!!!!! crop managements, xuhui
    ...
    !! DO j = 2, nvm
       ! IF (is_legume .AND. ok_LAIdev) THEN
        !ELSE
        !   Nbnf(:,j) = 0.0
...
!         N_add(:,:), kjpindex*nvm, horipft_index)
    ...
    CALL calc_N_limfert(kjpindex, clay, N_add, N_limfert) !! XZ ADD CLAY  
    ...
    ! before we calculate the N_impact, we must allocate the space for
    ! N_impact  XZ ADD
    ALLOCATE(N_impact(kjpindex,nvm),stat=ier)
    IF (ier/=0) THEN
        WRITE(numout,*) "ERROR IN ALLOCATION OF N_impact: ",ier
        STOP
    ENDIF
    N_impact = zero
    CALL calc_N_impact(kjpindex, clay, N_add, N_impact) !! XZ ADD
    IF (printlev>=4) THEN
        WRITE(numout,*) "Now we finish calculation of  the  N_impact"
    ENDIF
...
!!!!! for crops
               ...
               &             nger, nlev, ndrp,  nlax, &
               &             c_reserve, c_leafb, nmat, nrec, N_limfert, tday_counter, & !! XZ ADD
!!!!! end for crops
              ...
              IF (ok_LAIdev(j)) THEN
                  ...
                  DO i = 1, kjpindex
                      ...
                      call reprac_calc(tday_counter, nsen(i, j), nlax(i, j), nflo(i, j), nmat(i, j), nrec(i, j), nplt(i, j), in_cycle(i, j), nger(i, j), st2m_daily(i), nlev(i, j), &      ! IN
                                  ...
                                  SP_codetemprac(j), SP_tcmin(j), SP_tcmax(j), SP_codedyntalle(j), SP_tcxstop(j), SP_stlevamf(j), SP_stamflax(j), N_impact)     ! parameter XZ ADD
...
!    IF (ALLOCATED(ndrpobs))          DEALLOCATE(ndrpobs)
    ...
    IF (ALLOCATED(Nbnf)) DEALLOCATE(Nbnf)
    IF (ALLOCATED(N_limfert)) DEALLOCATE(N_limfert)
    IF (ALLOCATED(N_impact)) DEALLOCATE(N_impact)  !! XZ ADD
...
!ENDJCADD
! XZ ADD
    REAL(r_std),DIMENSION(kjpindex,nvm),INTENT(in)       :: N_impact
    ! 0.4 Local variables
...
! End of Edition by Xuhui, Mar. 16th 2011
  ...
  SUBROUTINE calc_N_limfert(npts,clay,N_fert_total,N_limfert)
 !! XZ ADD CLAY
...
!    N_limfert(:,:) = 1. + N_effect - N_effect * (0.75 ** (N_fert_total(:,:)/30))
    DO j=2, nvm
        ...
        ! ENDIF
        IF ( ok_LAIdev(j) ) THEN
            IF (0.0 <= clay(j) .AND. clay(j) < 0.2) THEN
                N_limfert(:,j) = 1.510955 / (1.0 + EXP(-0.004974724 * (N_fert_total(:,j) + 134.9772)))
            ELSE 
                N_limfert(:,j) = 1.640313 / (1.0 + EXP(-0.004976956 * (N_fert_total(:,j) + 89.57256)))
	ENDIF
        ELSE
            N_limfert(:,j) = 1.
        ENDIF
...
!! XZ ADD
SUBROUTINE calc_N_impact(npts,clay,N_fert_total,N_impact)
 !! XZ ADD CLAY
  INTEGER (i_std)                             , INTENT(in)  :: npts
  REAL(r_std), DIMENSION(npts)          , INTENT(in) :: clay
  REAL(r_std), DIMENSION(npts,nvm)          , INTENT(in) :: N_fert_total
  REAL(r_std), DIMENSION(npts,nvm)          , INTENT(out) :: N_impact
 
  INTEGER(i_std) :: k,j,ip
   
    DO j=2, nvm
        IF ( (SP_neffmax(j) .lt. zero) .AND. ( ok_LAIdev(j) ) ) THEN
            STOP 'calc_N_impact, negative N effect'
        ENDIF
        IF ( ok_LAIdev(j) ) THEN
            IF (0.0 <= clay(j) .AND. clay(j) < 0.2) THEN
                N_impact(:,j) = (1.0 + exp(-0.004749294 * (N_fert_total(:,j) + 100.0053))) / 1.621913
            ELSE 
                N_impact(:,j) = (1.0 + exp(-0.004582786 * (N_fert_total(:,j) + 50.2721))) / 1.794227
	ENDIF
        ELSE
            N_impact(:,j) = 1.
        ENDIF
